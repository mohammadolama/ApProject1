...
public class JsonBuilders {
   static ObjectMapper objectMapper=new ObjectMapper();
    ...
    public static void PlayerJsonBuilder(String username , Player player) throws IOException {
        String path=String.format("Jsons\\Players\\%s",username);
    ...
    public static void NewPlayerHeroBuilder(Player player) throws IOException {
        String path=String.format("Jsons\\Players\\%s",player.getUsername());
        ...
        objectMapper.writeValue(fileWriter,JsonReaders.NewPlayerHeroReader("rouge"));
        fileWriter.close();
    ...
    public static void HeroBuilder(Player player , Hero hero) throws IOException {
        String st=String.format("Jsons\\Players\\%s\\%s.json",player.getUsername(),hero.getName().toLowerCase());
        FileWriter fileWriter=new FileWriter(st);
        objectMapper.writeValue(fileWriter , hero);
        fileWriter.close();
    }
   public static void WeaponBuilder(Weapon weapon) throws IOException {
        String st=String.format("Jsons\\Cards\\%s.json",weapon.getName().toLowerCase());
        FileWriter fileWriter=new FileWriter(st);
        objectMapper.writeValue(fileWriter , weapon);
        fileWriter.close();
    }
   public static void SpellBuilder(Spell spell) throws IOException {
        String st=String.format("Jsons\\Cards\\%s.json",spell.getName().toLowerCase());
        FileWriter fileWriter=new FileWriter(st);
