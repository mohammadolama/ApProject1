...
public class Collections {
    ...
    static void Collections(Player player) throws IOException {
        ...
        while (true) {
            ...
            if (st.equalsIgnoreCase("Pcards")) {
                ...
                System.out.println(player123.getSelectedHero().getDcarts()+"\n");
    ...
    private static void Remove() throws IOException {
        while (true) {
            boolean flag = false;
            if (player123.getSelectedHero().getDcarts().size() == 0) {
            ...
            if (st.equals("back")) {
                ...
                ArrayList<Carts> ar = player123.getSelectedHero().getDcarts();
                for (Carts cartss : ar) {
                    if (st.equals(cartss.toString())) {
                        if (player123.getSelectedHero().getDcarts().size() >= 9) {
                            if (Info.WantToDo(st)) {
                                ar.remove(Carts.valueOf(st));
                                player123.getSelectedHero().UpdateAddPurchasedCards(Carts.valueOf(st));
                                ...
                                JsonBuilders.HeroBuilder(player123,player123.getSelectedHero());
                            }
                        } else {
                ...
                player123.getSelectedHero().setDcarts(ar);
            }
        }
    }
    ...
    private static void Add() throws IOException {
        while (true) {
            if (player123.getSelectedHero().getPcarts().size() == 0) {
            ...
            System.out.println(player123.getSelectedHero().getPcarts());
            String st = sc.next().toLowerCase();
            boolean flag = false;
            if (st.equals("back")) {
                JsonBuilders.PlayerJsonBuilder(player123.getUsername(), player123);
            ...
            for (Carts cartss : player123.getSelectedHero().getPcarts()) {
                if (st.equals(cartss.toString().toLowerCase())) {
                    if (player123.getSelectedHero().getDcarts().size() < 15) {
                        if (Info.WantToDo(st)) {
                            player123.getSelectedHero().UpdateAddDeckCards(Carts.valueOf(st));
                            player123.getSelectedHero().UpdateRemovePurchasedCards(Carts.valueOf(st));
    ...
    private static ArrayList<Carts> dv() {
        ...
        for (Carts cartss : player123.getSelectedHero().getDcarts()) {
            ar.add(cartss);
        }
        for (Carts cartss : player123.getSelectedHero().getPcarts()) {
            ar.add(cartss);
